...
- [x] Fix bump-caused CI breakage: pinned `golangci-lint v1.61.0` (built with go1.23) refuses the `go 1.26` directive → upgraded `ci.yml` to `golangci-lint-action@v8` + `golangci-lint v2.12.2` (built with go1.26). Verified `golangci-lint run ./...` = 0 issues on the root module locally.
